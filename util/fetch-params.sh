...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/LASTSParams"
    else
        PARAMS_DIR="$HOME/.lats-params"
    fi
fi
...
Failed to fetch the LATS zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
LATS - fetch-params.sh

This script will fetch the LATS zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common LATS zkSNARK parameters. Note that it is
