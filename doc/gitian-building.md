...
*Setup instructions for a gitian build of LATS Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the LATS
...
to the LATS GitHub release page.
...
- [Building LATS Core](#building-lats-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/lats-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/lats-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/lats-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/lats-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/lats-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/lats-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/lats-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@lats.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/lats-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/lats-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/lats-osx-*-build.assert
```
