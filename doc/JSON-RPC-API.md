...
Run ''latsd'' or ''lats-qt -server''. You can control it via the command-line lats-cli utility or by [http://json-rpc.org/wiki/specification HTTP JSON-RPC] commands.

You must create a lats.conf configuration file setting an rpcuser and rpcpassword; see [[Running LATS]] for details.

Now run:
  $ ./latsd -daemon
  LATS server starting
  $ ./lats-cli -rpcwait help
  # shows the help tex
...
If you are learning the API, it is a very good idea to use the test network (run latsd -testnet and lats-cli -testnet).

== JSON-RPC ==

Running LATS with the -server argument (or running latsd) tells it to function as a [http://json-rpc.org/wiki/specification HTTP JSON-RPC] server, but
...
Allowing arbitrary machines to access the JSON-RPC port (using the rpcallowip [[Running LATS|configuration option]]) is dangerous and '''strongly discouraged'''-- access should be strictly limited to trusted machines.
...
See the [[Proper Money Handling (JSON-RPC)|proper money handling page]] for notes on avoiding rounding errors when handling LATS values.
...
== Erlang ==
Get the rebar dependency from https://github.com/edescourtis/ebitcoind . By default the client will use the configuration in <code>$HOME/.lats/lats.conf</code> or you can instead specify a URI like this:
<source lang="erlang">ebitcoind:start_link(<<"http://user:password@localhost:45455/">>).</source>
...
The [http://jsonrpcphp.org/ JSON-RPC PHP] library also makes it very easy to connect to LATS.  For example:

<source lang="php">
  require_once 'jsonRPCClient.php';

  $lats = new jsonRPCClient('http://user:password@127.0.0.1:51473/');

  echo "<pre>\n";
  print_r($lats->getinfo()); echo "\n";
  echo "Received: ".$lats->getreceivedbylabel("Your Address")."\n";
  echo "</pre>";
</source>

'''Note:''' The jsonRPCClient library uses fopen() and will throw an exception saying "Unable to connect" if it receives a 404 or 500 error from latsd. This prevents you from being able to see error messages generated by latsd (as they are sent with status 404 or 500). The [https://github.com/aceat64/EasyBitcoin-PHP EasyBitcoin-PHP library] is similar in function to JSON-RPC PHP but does not have this issue.
...
The JSON::RPC package from CPAN can be used to communicate with LATS.  You must set the client's credentials; for example:
...
<source lang="perl">
  ...
  $client->ua->credentials(
     'localhost:51473', 'jsonrpc', 'user' => 'password'  # REPLACE WITH YOUR lats.conf rpcuser/rpcpassword
      );
...
The [https://github.com/btcsuite/btcrpcclient btcrpcclient package] can be used to communicate with LATS.  You must provide credentials to match the client you are communicating with.
