...
class ReorgStakeTest(LatsTestFramework):
    ...
    def check_money_supply(self, expected_lats):
        # verify that nodes have the expected LATS supply
        lats_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(lats_supply, [DecimalAmt(expected_lats)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that LATS supply was properly updated after the reorgs (including burned fee)
        self.log.info("Check LATS supply...")
