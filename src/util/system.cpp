...
const char * const LATS_CONF_FILENAME = "lats.conf";
const char * const LATS_MASTERNODE_CONF_FILENAME = "masternode.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager& am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "latsd -foo=bar
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\LATS
// Windows >= Vista: C:\Users\Username\AppData\Roaming\LATS
// Mac: ~/Library/Application Support/LATS
// Unix: ~/.lats
#ifdef WIN32
    // Windows
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\LATSParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\LATSParams
    // Mac: ~/Library/Application Support/LATSParams
    // Unix: ~/.lats-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "LATSParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectories(pathRet);
    return pathRet / "LATSParams";
#else
    // Unix
    return pathRet / ".lats-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/lats/sapling-spend.params";
        sapling_output = "/usr/share/lats/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/lats/sapling-spend.params";
            sapling_output = "/usr/local/share/lats/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf", LATS_MASTERNODE_CONF_FILENAME));
