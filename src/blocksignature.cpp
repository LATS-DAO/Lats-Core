// Copyright (c) 2017-2021 The LATS developers
...
bool CheckBlockSignature(const CBlock& block)
{
    ...
    /** Each block is signed by the private key of the input that is staked. This can be either zLATS or normal UTXO
     *  zLATS: Each zLATS has a keypair associated with it. The serial number is a hash of the public key.
     *  UTXO: The public key that signs must match the public key associated with the first utxo of the coinstake tx.
     */
    CPubKey pubkey;
    bool fzLATSStake = block.vtx[1]->vin[0].IsZerocoinSpend();
    if (fzLATSStake) {
        libzerocoin::CoinSpend spend = ZLATSModule::TxInToZerocoinSpend(block.vtx[1]->vin[0]);
