...
#if defined(NDEBUG)
#error "LATS cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ulats) are considered zero fee (for relaying, mining and transaction creation)
...
CAmount GetBlockValue(int nHeight)
{
    ...
    // Testnet high-inflation blocks [2, 200] with value 250k FRAG
	...
	if (nHeight >= 1 && nHeight <= 2000) {
        ret = COIN * 100;
	} else if (nHeight < 192000) {
        ret = COIN * 9;
    }
    else if (nHeight <= 300152) {
        ret = COIN * 4.5;
    }
    else if (nHeight <= 800153) {
        ret = COIN * 2.5;
    }
    else if (nHeight <= 1600154) {
        ret = COIN * 1.5;
    }
    else if (nHeight <= 3000155) {
        ret = COIN * 1;
    }
    else {
        ret = COIN * 0;
...
int64_t GetMasternodePayment(int nHeight)
{
    ...
    else if(nHeight <= 9900157){
        ret = GetBlockValue(nHeight) * 60 / 100;
    }
    else if(nHeight <= 16400158){
...
{
    util::ThreadRename("lats-scriptch");
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (consensus.NetworkUpgradeActive(pindex->nHeight, Consensus::UPGRADE_ZC_V2) &&
        ...
        if (pindex->nHeight % 50000 == 0) {
            ZLATSModule::CleanCoinSpendsCache();
        }
...
bool CheckWork(const CBlock& block, const CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // Lats Specific reference to the block with the wrong threshold was used.
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nLatsBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nLatsBadBlockBits)) {
            // accept LATS block minted with incorrect proof of work threshold
            return true;
        }
                                     ...
                                     std::set<CBigNum>& spent_serials)
{
    ...
    const bool zlatsActive = consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC);
    const bool publicZlatsActive = consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_PUBLIC);
    ...
    for (size_t i = 1; i < block.vtx.size(); i++) {
        ...
        for (const CTxIn& in: tx->vin) {
            ...
            if (isPublicSpend && (!publicZlatsActive || v5Active)) {
            ...
            if (isPrivZerocoinSpend && (!zlatsActive || publicZlatsActive)) {
            ...
            if (isPrivZerocoinSpend || isPublicSpend) {
                ...
                if (isPublicSpend) {
                    ...
                    if (!ZLATSModule::ParseZerocoinPublicSpend(in, *tx, state, publicSpend)){
                        return false;
                    }
                    spend = publicSpend;
                } else {
                    spend = ZLATSModule::TxInToZerocoinSpend(in);
                }
        ...
        for (auto it = bl.vtx.rbegin(); it != bl.vtx.rend(); ++it) {
            ...
            for (const CTxIn& in: tx->vin) {
                ...
                if (!in.IsZerocoinSpend()) {
                    ...
                    // zerocoin serial
                    const CBigNum& s = ZLATSModule::TxInToZerocoinSpend(in).getCoinSerialNumber();
...
static bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, const FlatFilePos* dbp) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (isPoS) {
        ...
        if (coinstake_in.IsZerocoinSpend()) {
            libzerocoin::CoinSpend spend = ZLATSModule::TxInToZerocoinSpend(coinstake_in);
            if (!ContextualCheckZerocoinSpend(coinstake, &spend, pindex->nHeight)) {
                return state.DoS(100,error("%s: main chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                        coinstake.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zlats");
            }
        }
