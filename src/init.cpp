...
// Copyright (c) 2015-2021 The LATS developers
 ...
 */
const char * const LATS_PID_FILENAME = "lats.pid";

fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", LATS_PID_FILENAME));
...
void Shutdown()
{
    ...
    util::ThreadRename("lats-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf("Specify configuration file (default: %s)", LATS_CONF_FILENAME));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf("Specify pid file (default: %s)", LATS_PID_FILENAME));
#endif
...
{
    util::ThreadRename("lats-loadblk");
    CImportingNow imp;
...
/** Sanity checks
 *  Ensure that LATS is running in a usable environment with all
...
{
    // Make sure only a single LATS process is using the data directory.
...
bool AppInitMain()
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", LATS_CONF_FILENAME)).string());
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile because if LATS is started in the future "
                  "from a different location. It will be unable to locate the current data files. There could "
                  "also be data loss if LATS is started while in a temporary directory.\n",
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                //LATS specific: zerocoin and spork DB's
                ...
                // LATS: load previous sessions sporks if we have them.
