...
#include "qt/lats/forms/ui_addresseswidget.h"
#include "qt/lats/addressfilterproxymodel.h"
#include "qt/lats/addresslabelrow.h"
#include "qt/lats/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(LATSGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto latsAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(latsAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(latsAdd);
        if (walletModel->updateAddressBookLabels(latsAdd, label.toUtf8().constData(),
