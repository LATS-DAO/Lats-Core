...
#include "qt/lats/defaultdialog.h"
#include "shutdown.h"
...
const QString LATSGUI::DEFAULT_WALLET = "~Default";

LATSGUI::LATSGUI(const NetworkStyle* networkStyle, QWidget* parent) :
        QMainWindow(parent),
        clientModel(0){
...
void LATSGUI::createActions(const NetworkStyle* networkStyle)
{
    ...
    connect(toggleHideAction, &QAction::triggered, this, &LATSGUI::toggleHidden);
...
void LATSGUI::connectActions()
{
    ...
    connect(topBar, &TopBar::showHide, this, &LATSGUI::showHide);
    connect(topBar, &TopBar::themeChanged, this, &LATSGUI::changeTheme);
    connect(topBar, &TopBar::onShowHideColdStakingChanged, navMenu, &NavMenuWidget::onShowHideColdStakingChanged);
    connect(settingsWidget, &SettingsWidget::showHide, this, &LATSGUI::showHide);
    connect(sendWidget, &SendWidget::showHide, this, &LATSGUI::showHide);
    connect(receiveWidget, &ReceiveWidget::showHide, this, &LATSGUI::showHide);
    connect(addressesWidget, &AddressesWidget::showHide, this, &LATSGUI::showHide);
    connect(masterNodesWidget, &MasterNodesWidget::showHide, this, &LATSGUI::showHide);
    connect(masterNodesWidget, &MasterNodesWidget::execDialog, this, &LATSGUI::execDialog);
    connect(coldStakingWidget, &ColdStakingWidget::showHide, this, &LATSGUI::showHide);
    connect(coldStakingWidget, &ColdStakingWidget::execDialog, this, &LATSGUI::execDialog);
    connect(governancewidget, &GovernanceWidget::showHide, this, &LATSGUI::showHide);
    connect(governancewidget, &GovernanceWidget::execDialog, this, &LATSGUI::execDialog);
    connect(settingsWidget, &SettingsWidget::execDialog, this, &LATSGUI::execDialog);
}


void LATSGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
#ifndef Q_OS_MAC
...
void LATSGUI::handleRestart(QStringList args)
{
...
void LATSGUI::setClientModel(ClientModel* _clientModel)
{
    ...
    if (this->clientModel) {
        ...
        connect(clientModel, &ClientModel::message, this, &LATSGUI::message);
...
void LATSGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
    ...
    connect(trayIcon, &QSystemTrayIcon::activated, this, &LATSGUI::trayIconActivated);
#else
    // Note: On macOS, the Dock icon is used to provide the tray's functionality.
    MacDockIconHandler* dockIconHandler = MacDockIconHandler::instance();
    connect(dockIconHandler, &MacDockIconHandler::dockIconClicked, this, &LATSGUI::macosDockIconActivated);
...
#ifndef Q_OS_MAC
void LATSGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
#else
void LATSGUI::macosDockIconActivated()
 {
     show();
     activateWindow();
 }
...
void LATSGUI::changeEvent(QEvent* e)
{
    ...
    if (e->type() == QEvent::WindowStateChange) {
        if (clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray()) {
            ...
            if (!(wsevt->oldState() & Qt::WindowMinimized) && isMinimized()) {
                QTimer::singleShot(0, this, &LATSGUI::hide);
                e->ignore();
            } else if ((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized()) {
                QTimer::singleShot(0, this, &LATSGUI::show);
                e->ignore();
            }
        }
    }
...
void LATSGUI::closeEvent(QCloseEvent* event)
{
#ifndef Q_OS_MAC // Ignored on Mac
...
void LATSGUI::messageInfo(const QString& text)
{
...
void LATSGUI::message(const QString& title, const QString& message, unsigned int style, bool* ret)
{
...
bool LATSGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
...
void LATSGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if (!clientModel)
        return;
...
void LATSGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void LATSGUI::detectShutdown()
{
    if (ShutdownRequested()) {
        if (rpcConsole)
            rpcConsole->hide();
...
void LATSGUI::goToSend()
{
    showTop(sendWidget);
}

void LATSGUI::goToAddresses()
{
    showTop(addressesWidget);
}

void LATSGUI::goToMasterNodes()
{
    showTop(masterNodesWidget);
}

void LATSGUI::goToColdStaking()
{
    showTop(coldStakingWidget);
}

void LATSGUI::goToGovernance()
{
    showTop(governancewidget);
}

void LATSGUI::goToSettings(){
    showTop(settingsWidget);
}

void LATSGUI::goToSettingsInfo()
{
    navMenu->selectSettings();
...
void LATSGUI::goToReceive()
{
    showTop(receiveWidget);
}

void LATSGUI::openNetworkMonitor()
{
    settingsWidget->openNetworkMonitor();
}

void LATSGUI::showTop(QWidget* view)
{
...
void LATSGUI::changeTheme(bool isLightTheme)
{
...
void LATSGUI::resizeEvent(QResizeEvent* event)
{
    // Parent..
...
bool LATSGUI::execDialog(QDialog *dialog, int xDiv, int yDiv)
{
    return openDialogWithOpaqueBackgroundY(dialog, this);
}

void LATSGUI::showHide(bool show)
{
...
int LATSGUI::getNavWidth()
{
    return this->navMenu->width();
}

void LATSGUI::openFAQ(SettingsFaqWidget::Section section)
{
    showHide(true);
...
#ifdef ENABLE_WALLET
void LATSGUI::setGovModel(GovernanceModel* govModel)
{
    if (!stackedContainer || !clientModel) return;
    governancewidget->setGovModel(govModel);
}

void LATSGUI::setMNModel(MNModel* _mnModel)
{
    if (!stackedContainer || !clientModel) return;
    mnModel = _mnModel;
...
bool LATSGUI::addWallet(const QString& name, WalletModel* walletModel)
{
    ...
    // Connect actions..
    connect(walletModel, &WalletModel::message, this, &LATSGUI::message);
    connect(masterNodesWidget, &MasterNodesWidget::message, this, &LATSGUI::message);
    connect(coldStakingWidget, &ColdStakingWidget::message, this, &LATSGUI::message);
    connect(topBar, &TopBar::message, this, &LATSGUI::message);
    connect(sendWidget, &SendWidget::message,this, &LATSGUI::message);
    connect(receiveWidget, &ReceiveWidget::message,this, &LATSGUI::message);
    connect(addressesWidget, &AddressesWidget::message,this, &LATSGUI::message);
    connect(governancewidget, &GovernanceWidget::message,this, &LATSGUI::message);
    connect(settingsWidget, &SettingsWidget::message, this, &LATSGUI::message);

    // Pass through transaction notifications
    connect(dashboard, &DashboardWidget::incomingTransaction, this, &LATSGUI::incomingTransaction);

    return true;
}

bool LATSGUI::setCurrentWallet(const QString& name)
{
    // Single wallet supported.
    return true;
}

void LATSGUI::removeAllWallets()
{
    // Single wallet supported.
}

void LATSGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address)
{
...
static bool ThreadSafeMessageBox(LATSGUI* gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void LATSGUI::subscribeToCoreSignals()
{
...
void LATSGUI::unsubscribeFromCoreSignals()
{
