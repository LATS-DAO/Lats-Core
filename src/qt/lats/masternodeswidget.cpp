...
#include "qt/lats/forms/ui_masternodeswidget.h"

#include "qt/lats/qtutils.h"
#include "qt/lats/mnrow.h"
#include "qt/lats/mninfodialog.h"
#include "qt/lats/masternodewizarddialog.h"
...
MasterNodesWidget::MasterNodesWidget(LATSGUI *parent) :
    PWidget(parent),
...
void MasterNodesWidget::onCreateMNClicked()
{
    ...
    if (walletModel->getBalance() <= mnCollateralAmount) {
        inform(tr("Not enough balance to create a masternode, %1 required.")
            .arg(GUIUtil::formatBalance(mnCollateralAmount, BitcoinUnits::LATS)));
        return;
    }
    showHideOp(true);
