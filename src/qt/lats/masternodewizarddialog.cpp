...
#include "qt/lats/forms/ui_masternodewizarddialog.h"
    ...
    mnModel(_mnModel)
{
    ...
    ui->labelMessage1a->setText(formatHtmlContent(
                formatParagraph(tr("To create a LATS Masternode you must dedicate %1 (the unit of LATS) "
    ...
    if (walletModel->isRegTestNetwork()) {
        ui->lineEditPort->setText("47454");
    } else if (walletModel->isTestNetwork()) {
        ui->lineEditPort->setText("46454");
    } else {
