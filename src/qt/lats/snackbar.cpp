...
#include "qt/lats/forms/ui_snackbar.h"
#include "qt/lats/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(LATSGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &LATSGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
