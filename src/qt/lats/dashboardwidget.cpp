...
#include "qt/lats/dashboardwidget.h"
#include "qt/lats/forms/ui_dashboardwidget.h"
#include "qt/lats/sendconfirmdialog.h"
#include "qt/lats/txrow.h"
...
DashboardWidget::DashboardWidget(LATSGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareLats, "square-chart-lats");
    setCssProperty(ui->labelSquareMN, "square-chart-mn");
    setCssProperty(ui->labelLats, "text-chart-lats");
    ...
    setCssProperty(ui->labelAmountLats, "text-stake-lats-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &LATSGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("LATS")
    updateDisplayUnit();
}
...
QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isLats = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::MNReward;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair LATS, MN Reward
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            lats = (pair.first != 0) ? pair.first / 100000000 : 0;
            mn = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalLats += pair.first;
            chartData->totalMN += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesLats.append(lats);
        chartData->valuesMN.append(mn);

        int max = std::max(lats, mn);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(55,72,167));
    set1->setColor(QColor(89,198,255));
    ...
    set0->append(chartData->valuesLats);
    set1->append(chartData->valuesMN);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalLats > 0 || chartData->totalMN > 0) {
        setCssProperty(ui->labelAmountLats, "text-stake-lats");
        setCssProperty(ui->labelAmountMN, "text-stake-mn");
    } else {
        setCssProperty(ui->labelAmountLats, "text-stake-lats-disable");
        setCssProperty(ui->labelAmountMN, "text-stake-mn-disable");
    }
    forceUpdateStyle({ui->labelAmountLats, ui->labelAmountMN});
    ui->labelAmountLats->setText(GUIUtil::formatBalance(chartData->totalLats, nDisplayUnit));
    ...
    if (fShowCharts) {
        if (!stakesFilter) {
            ...
            stakesFilter->setTypeFilter(TransactionFilterProxy::TYPE(TransactionRecord::StakeMint) |
                                        ...
                                        TransactionFilterProxy::TYPE(TransactionRecord::StakeZLATS) |
