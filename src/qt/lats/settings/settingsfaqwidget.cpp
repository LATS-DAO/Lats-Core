...
#include "qt/lats/settings/settingsfaqwidget.h"
#include "qt/lats/settings/forms/ui_settingsfaqwidget.h"
#include "qt/lats/mnmodel.h"
#include "qt/lats/qtutils.h"

#include <QScrollBar>
#include <QMetaObject>

SettingsFaqWidget::SettingsFaqWidget(LATSGUI* parent, MNModel* mnModel) :
    QDialog(parent),
    ui(new Ui::SettingsFaqWidget)
{
    ...
    setCssProperty({
           ...
           ui->labelNumber_UnspendableLATS,
    ...
    setCssProperty({
              ...
              ui->labelSubtitle_UnspendableLATS,
    ...
    setCssProperty({
              ...
              ui->labelContent_UnspendableLATS,
    ...
    QString introContent = formatFAQContent(
        formatFAQParagraph(
            tr("LATS is a form of digital online money using blockchain technology "
        ...
        formatFAQParagraph(
            tr("LATS utilizes a Proof of Stake (PoS) consensus system algorithm, "
    ...
    QString unspendableLATSContent = formatFAQContent(
        formatFAQParagraph(
            tr("Newly received LATS requires 6 confirmations on the network "
               "to become eligible for spending which can take ~6 minutes.")) +
        formatFAQParagraph(
            tr("Your LATS wallet also needs to be completely synchronized "
               "to see and spend balances on the network.")));
    ui->labelContent_UnspendableLATS->setText(unspendableLATSContent);
    ...
    QString stakeContent = formatFAQContent(
        formatFAQOrderedList(
            ...
            formatFAQListItem(tr("You must have a balance of LATS with a minimum of 600 confirmations.")) +
        ...
        formatFAQParagraph(
            tr("You can see the status of staking in the wallet by mousing over the "
               ...
               "will confirm that staking is active.")
                .arg("lats-cli", "<span style=\"font-style:italic\">getstakingstatus</span>")));
    ...
    QString supportContent = formatFAQContent(
        formatFAQParagraph(
            tr("We have support channels in most of our official chat groups, for example %1")
                .arg("<a style='color: #b088ff' href='https://discord.gg/cGsXc3gKeQ'>" + tr("#support in our Discord") + "</a>.")));
    ...
    QString masternodeContent = formatFAQContent(
        formatFAQParagraph(
            tr("A masternode is a computer running a full node %1 wallet with a "
               ...
               "regularly. These services include:")
                .arg(PACKAGE_NAME)
                .arg(GUIUtil::formatBalance(mnModel->getMNCollateralRequiredAmount(), BitcoinUnits::LATS)) +
        ...
        formatFAQParagraph(
            tr("Masternode Perks:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("Participate in LATS Governance")) +
                formatFAQListItem(tr("Earn Masternode Rewards")) +
                formatFAQListItem(tr("Commodity option for future sale")) +
                formatFAQListItem(tr("Help secure the LATS network")))) +
        formatFAQParagraph(
            tr("Requirements:") +
            formatFAQUnorderedList(
    ...
    connect(ui->pushButton_UnspendableLATS, &QPushButton::clicked, [this](){onFaqClicked(ui->widget_UnspendableLATS);});
    ...
    if (parent)
        connect(parent, &LATSGUI::windowResizeEvent, this, &SettingsFaqWidget::windowResizeEvent);
}
