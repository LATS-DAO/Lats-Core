...
#include "qt/lats/send.h"
#include "qt/lats/forms/ui_send.h"
#include "qt/lats/addnewcontactdialog.h"
#include "qt/lats/qtutils.h"
#include "qt/lats/sendchangeaddressdialog.h"
#include "qt/lats/optionbutton.h"
#include "qt/lats/sendconfirmdialog.h"
#include "qt/lats/guitransactionsutils.h"
#include "qt/lats/loadingdialog.h"
...
SendWidget::SendWidget(LATSGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    ui->btnUri->setSubTitleClassAndText("text-subtitle", tr("Parse a LATS URI"));
    ...
    setCssProperty(coinIcon, "coin-icon-lats");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onLATSSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onLATSSelected(false);});
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent LATSs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res) {
                inform(tr("Error generating address to shield LATSs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent LATSs to shield."));
    }
}
...
void SendWidget::onLATSSelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto latsAdd = Standard::DecodeDestination(address.toStdString(), isStakingAddr);

        if (!Standard::IsValidDestination(latsAdd) || isStakingAddr) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(latsAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(latsAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
