...
#include "qt/lats/splash.h"
#include "qt/lats/welcomecontentwidget.h"
...
#ifdef ENABLE_WALLET
#include "qt/lats/governancemodel.h"
...
{
    return QCoreApplication::translate("lats-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in lats.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in lats.qrc)
...
/** Class encapsulating LATS Core startup and shutdown.
...
public:
    ...
    /// Get window identifier of QMainWindow (LATSGUI)
...
{
    window = new LATSGUI(networkStyle, nullptr);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &LATSGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &LATSGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (!vpwallets.empty()) {
            ...
            window->addWallet(LATSGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(LATSGUI::DEFAULT_WALLET);
        ...
        // LATS: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &LATSGUI::handlePaymentRequest);
        connect(window, &LATSGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(nullptr, "Runaway exception", QObject::tr("A fatal error occurred. LATS can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    ...
    /// 6. Determine availability of data directory and parse lats.conf
    ...
    // lats: links repeatedly have their payment requests routed to this process:
