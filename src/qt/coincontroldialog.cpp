...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select LATS Outputs to Spend" :
            "Select Shielded LATS to Spend");
    ...
    int nDisplayUnit = BitcoinUnits::LATS;
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show LATS address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
