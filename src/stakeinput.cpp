...
CLatsStake* CLatsStake::NewLatsStake(const CTxIn& txin, int nHeight, uint32_t nTime)
{
    if (txin.IsZerocoinSpend()) {
        error("%s: unable to initialize CLatsStake from zerocoin spend", __func__);
        return nullptr;
    }
    ...
    if (!coin.IsSpent()) {
        ...
        // All good
        return new CLatsStake(coin.out, txin.prevout, pindexFrom);
    }
    ...
    // All good
    return new CLatsStake(txPrev->vout[txin.prevout.n], txin.prevout, pindexFrom);
}

bool CLatsStake::GetTxOutFrom(CTxOut& out) const
{
    out = outputFrom;
    return true;
}

CTxIn CLatsStake::GetTxIn() const
{
    return CTxIn(outpointFrom.hash, outpointFrom.n);
}

CAmount CLatsStake::GetValue() const
{
    return outputFrom.nValue;
}

CDataStream CLatsStake::GetUniqueness() const
{
    //The unique identifier for a LATS stake is the outpoint
...
const CBlockIndex* CLatsStake::GetIndexFrom() const
{
    // Sanity check, pindexFrom is set on the constructor.
    if (!pindexFrom) throw std::runtime_error("CLatsStake: uninitialized pindexFrom");
    return pindexFrom;
}
