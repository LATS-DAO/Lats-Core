...
BOOST_AUTO_TEST_CASE(testVectors) {
        ...
        BOOST_CHECK(m.depth == 0);
        BOOST_CHECK(m.parentFVKTag == 0);
        BOOST_CHECK(m.childIndex == 0);
        BOOST_CHECK(m.chaincode ==
        ...
        diversifier_t dTestVector{ 0xd8, 0x62, 0x1b, 0x98, 0x1c, 0xf3, 0x00, 0xe9, 0xd4, 0xcc, 0x89 };
        BOOST_CHECK(m.ToXFVK().DefaultAddress().d == dTestVector);

        auto m_1 = m.Derive(1);
        BOOST_CHECK(m_1.depth == 1);
        BOOST_CHECK(m_1.parentFVKTag == 0x3a71c214);
        BOOST_CHECK(m_1.childIndex == 1);
        BOOST_CHECK(
                m_1.chaincode ==
        ...
        dTestVector = diversifier_t({ 0x8b, 0x41, 0x38, 0x32, 0x0d, 0xfa, 0xfd, 0x7b, 0x39, 0x97, 0x81 });
        BOOST_CHECK(m_1.ToXFVK().DefaultAddress().d == dTestVector);

        auto m_1_2h = m_1.Derive(2 | ZIP32_HARDENED_KEY_LIMIT);
        BOOST_CHECK(m_1_2h.depth == 2);
        BOOST_CHECK(m_1_2h.parentFVKTag == 0x079e99db);
        BOOST_CHECK(m_1_2h.childIndex == (2 | ZIP32_HARDENED_KEY_LIMIT));
        BOOST_CHECK(
                m_1_2h.chaincode ==
        ...
        dTestVector = diversifier_t({ 0xe8, 0xd0, 0x37, 0x93, 0xcd, 0xd2, 0xba, 0xcc, 0x9c, 0x70, 0x41 });
        BOOST_CHECK(m_1_2h.ToXFVK().DefaultAddress().d == dTestVector);

        auto m_1_2hv = m_1_2h.ToXFVK();
        BOOST_CHECK(m_1_2hv.depth == 2);
        BOOST_CHECK(m_1_2hv.parentFVKTag == 0x079e99db);
        BOOST_CHECK(m_1_2hv.childIndex == (2 | ZIP32_HARDENED_KEY_LIMIT));
        BOOST_CHECK(
                m_1_2hv.chaincode ==
                uint256S("35d4a883737742ca41a4baa92323bdb3c93dcb3b462a26b039971bedf415ce97"));
        BOOST_CHECK(
        ...
        BOOST_CHECK(
                m_1_2hv.dk ==
                uint256S("377bb062dce7e0dcd8a0054d0ca4b4d1481b3710bfa1df12ca46ff9e9fa1eda3"));
        BOOST_CHECK(m_1_2hv.DefaultAddress() == m_1_2h.ToXFVK().DefaultAddress());

        // Hardened derivation from an xfvk fails
        BOOST_CHECK(!m_1_2hv.Derive(3 | ZIP32_HARDENED_KEY_LIMIT));

        // Non-hardened derivation succeeds
        auto maybe_m_1_2hv_3 = m_1_2hv.Derive(3);
        BOOST_CHECK(maybe_m_1_2hv_3);

        auto m_1_2hv_3 = maybe_m_1_2hv_3.get();
        BOOST_CHECK(m_1_2hv_3.depth == 3);
        BOOST_CHECK(m_1_2hv_3.parentFVKTag == 0x7583c148);
        BOOST_CHECK(m_1_2hv_3.childIndex == 3);
        BOOST_CHECK(
                m_1_2hv_3.chaincode ==
        ...
        dTestVector = diversifier_t({ 0x03, 0x0f, 0xfb, 0x26, 0x3a, 0x93, 0x9e, 0x23, 0x0e, 0x96, 0xdd });
        BOOST_CHECK(m_1_2hv_3.DefaultAddress().d == dTestVector);
}
