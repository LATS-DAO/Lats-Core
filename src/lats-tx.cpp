...
static bool AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help")) {
        ...
        std::string strUsage = PACKAGE_NAME " lats-tx utility version " + FormatFullVersion() + "\n\n" +
                               "Usage:  lats-tx [options] <hex-tx> [commands]  Update hex-encoded lats transaction\n" +
                               "or:     lats-tx [options] -create [commands]   Create hex-encoded lats transaction\n" +
                               "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded lats transaction
