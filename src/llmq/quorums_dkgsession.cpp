...
// Copyright (c) 2022 The LATS developers
...
CDKGLogger::CDKGLogger(const std::string& _llmqTypeName, const uint256& _quorumHash, int _height, bool _areWeMember, const std::string& _func) :
    CBatchedLogger(g_logger, BCLog::DKG, strprintf("QuorumDKG(type=%s, height=%d, member=%d, func=%s)", _llmqTypeName, _height, _areWeMember, _func))
{
}
...
void CDKGSession::SendJustification(CDKGPendingMessages& pendingMessages, const std::set<uint256>& forMembers)
{
    ...
    qj.sig = activeMasternodeManager->OperatorKey()->Sign(qj.GetSignHash());
...
void CDKGSession::SendCommitment(CDKGPendingMessages& pendingMessages)
{
    ...
    uint256 commitmentHash = utils::BuildCommitmentHash((Consensus::LLMQType)qc.llmqType, qc.quorumHash, qc.validMembers, qc.quorumPublicKey, qc.quorumVvecHash);

    if (lieType == 2) {
        (*commitmentHash.begin())++;
    }

    qc.sig = activeMasternodeManager->OperatorKey()->Sign(commitmentHash);
...
std::vector<CFinalCommitment> CDKGSession::FinalizeCommitments()
{
    ...
    for (const auto& p : commitmentsMap) {
        ...
        uint256 commitmentHash = utils::BuildCommitmentHash((Consensus::LLMQType)fqc.llmqType, fqc.quorumHash, fqc.validMembers, fqc.quorumPublicKey, fqc.quorumVvecHash);
