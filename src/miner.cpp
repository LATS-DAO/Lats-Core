...
bool ProcessBlockFound(const std::shared_ptr<const CBlock>& pblock, CWallet& wallet, std::unique_ptr<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("LATSMiner : generated block is stale");
    }
    ...
    if (!res || sc.stateErrorFound()) {
        return error("LATSMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("LATSMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("lats-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running LATSMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("LATSMiner exception");
    } catch (...) {
        LogPrintf("LATSMiner exception");
    }

    LogPrintf("LATSMiner exiting\n");
}
